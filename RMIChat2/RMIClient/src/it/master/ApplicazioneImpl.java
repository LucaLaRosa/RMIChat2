...
import it.chat.impl.WindowChat;
import it.chat.impl.WindowUsers;
import it.rmi.interfacce.ClientInterface;
import it.rmi.interfacce.ServerInterface;
import it.rmiClient.impl.ClientImpl;
import it.utility.Encryptor;
...
public class ApplicazioneImpl implements Applicazione{
	...
	public ApplicazioneImpl() {
		...
		try 
		{
			...
			System.out.println("Finisco di creare il client, da qui è un casino");
			
			this.server = (ServerInterface) client.getServer();
			
			System.out.println("info: onstructor ApplicazioneImpl() finished with success.");
	...
	public void registraFacebook(String user, String pwd) {
		
		try {
			
			 this.server.registraFacebook(user, pwd, client);
